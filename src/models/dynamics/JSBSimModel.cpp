...
int JSBSimModel::getEngNozzle(double* const pla, const int max) const
{
    ...
    for (int i = 0; i < num; i++) {
        ...
        case JSBSim::FGEngine::etTurboprop:
            {
                // For JSBSim::FGTurboProp, NozzlePosition is const equal to 1.0
                // JSBSim::FGTurboProp* eng1{static_cast<JSBSim::FGTurboProp*>(eng)};
                pla[i] = static_cast<double>(1.0 * 100.0);
            }
            break;
        case JSBSim::FGEngine::etPiston:
        case JSBSim::FGEngine::etRocket:
...
void JSBSimModel::reset()
{
    ...
    if (fdmex == nullptr) {
        ...
        fdmex->SetAircraftPath(SGPath(RootDir + "aircraft"));
        fdmex->SetEnginePath(SGPath(RootDir + "engine"));
        fdmex->SetSystemsPath(SGPath(RootDir + "systems")); // JSBSim-1.0 or after only
